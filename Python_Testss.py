...
def show_library_management():
    ...
    label_search = tk.Label(root, text="Nhập tên sách muốn tìm kiếm:",bg='#14C8EC')
    label_search.place(x=150, y=420)

    entry_search = tk.Entry(root)
    entry_search.place(x=100, y=450)

    button_search = tk.Button(root, text="Tìm kiếm", command=GUIshare_book)
...
def GUIshare_book():
    ...
    new_window.geometry(f"{200}x{300}")
    ...
    def search_book(keyword, listbox):
        ...
        for book in library.books:
            ...
            if keyword.lower() in book.name.lower() or keyword.lower() in book.author.lower():
                found_books.append(f"{a}) Name: {book.name}, Author: {book.author}")
        if found_books:
            for book in found_books:
